...
include_once '../view/mth_search_admin.php';
...
$max_lines = GlobalParameter::$applicationConfig['mthAdminNumLines'];

$res_view = new MethodResultView($db_conn);
$owner_id = 0;
$param_missing = 
    empty($_POST) ||
    empty($_POST['mth_ownership']) ||
    empty($_POST['curr_usr_id']) ||
    (($_POST['mth_ownership'] == 'O') && empty($_POST['mth_author']));
...
else
{
    ...
    {
        $owner_id = $_POST['curr_usr_id'];  
    }
    else
    {
        $owner_id = $_POST['mth_author'];
    }
    
    $res_view->initAdminListStmt($owner_id);
    ...
    $adm_view = new MethodAdminResultView($cur_usr_id > 0, $_POST['curr_usr_id']);
    
    foreach ($res_view->lines as $line)
    {
        $adm_view->addMethod($line);
    }
    $adm_view->numRemaining($res_view->getNumRemaining());
    $adm_view->renderHtml();
    $adm_view->outputHtml();
}
?>