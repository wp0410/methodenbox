...
/**
 * Returns the HTML string created for a menu item
 * 
 * @param          $mi_name  Name of the menu item (displayed text)
 * @param          $mi_ref   URL referenced by the menu item
 * @param          $show     Indicates whether or not the item shall be visible
 * 
 * @return         HTML text representation of the menu item
 */
function menu_item($mi_name, $mi_ref, $show)
{
    if ($show)
    {
        return '<li><a href="' . $mi_ref . '">' . $mi_name . '</a></li>';
    }
    else
    {
        return '';
 ...
 * 
 * @param          $usr_is_authenticated    Flag indicating the authentication
 *                                          status of the current user
 * @param          $current_form            File name of the current form
 */
function create_menu($usr_is_authenticated, $current_form)
{
    ...
    foreach(GlobalParam::$main_menu as $menu_item)
    {
        ...
        echo menu_item($menu_item['item_name'], $item_ref, $show_item);
    }
